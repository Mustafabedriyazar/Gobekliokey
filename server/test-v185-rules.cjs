...
T('mC-ok',!!(sr&&sr.ok)&&sr.turnEnded===false);
T('mC-state',s9.turnState==='DRAW'&&s9.turnIndex===0&&s9.players[0].hasDrawn===false&&!s9.pending);
T('mC-uid',!!s9.currentDiscard&&s9.currentDiscard.tile.uid==='SRX');
T('mC-zero',(s9.players[0].handPenalty||0)===hp9&&s9.players[0].rack.length===rl9);
...
mL.ftC=1;
console.log('v185-rules9: '+pass+' PASS '+fail+' FAIL '+(F.length?F.join(','):''));
