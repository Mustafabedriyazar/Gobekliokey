...
function a_okeyTake(p,meldId,candUid,jokUid){ var g=guard(p,["ACTION"]);if(g)return{ok:false,err:g}; var P=st.players[p];if(!P||!P.hasDrawn)return{ok:false,err:"once tas cekmelisin"}; var m=null;for(var i=0;i<st.melds.length;i++)if(st.melds[i].id===meldId){m=st.melds[i];break} if(!m)return{ok:false,err:"per bulunamadi"}; var ci=-1;for(i=0;i<P.rack.length;i++)if(String(P.rack[i].uid)===String(candUid)){ci=i;break} if(ci<0)return{ok:false,err:"tas elinde degil"}; var cand=P.rack[ci]; if(isJok(cand,st))return{ok:false,err:"okey ile okey alinmaz"}; var ji=-1;for(i=0;i<m.tiles.length;i++)if(isJok(m.tiles[i],st)){ji=i;break} if(ji<0)return{ok:false,err:"perde okey yok"}; if(jokUid!=null){var jf=-1;for(i=0;i<m.tiles.length;i++)if(String(m.tiles[i].uid)===String(jokUid)&&isJok(m.tiles[i],st)){jf=i;break} if(jf<0)return{ok:false,err:"hedef okey bulunamadi"}; ji=jf;} var jok=m.tiles[ji]; var _rep=jok.rep||null;if(_rep&&(m.kind==="pair"||m.form==="female")){var _cf=meldFace(cand,st);if(!_cf||_cf.color!==_rep.color||_cf.num!==_rep.num)return{ok:false,err:"yalniz okeyin temsil ettigi dogal tas ile degistirilebilir"}} var trial=m.tiles.slice();trial[ji]=cand; var ok2=false; if(m.kind==="series"){var tv2=tableMeldValid({id:m.id,kind:"series",tiles:trial,openLen:m.openLen});ok2=!!(tv2&&tv2.kind==="series")} else if(m.kind==="pair"){var gv=grpValid(trial);ok2=!!(gv&&gv.kind==="pair")} if(!ok2)return{ok:false,err:"aday tas okeyin yerini legal dolduramiyor"}; m.tiles[ji]=cand;P.rack.splice(ci,1);P.rack.push(jok);P.okeyLockUid=jok.uid;P.okeyLockKey=st.handIndex+":"+st.turnCount; ev("OKEY_TAKE",p,{meld:m.id,slot:ji,inUid:cand.uid,outUid:jok.uid}); return{ok:true,meld:m.id,slot:ji,tookUid:jok.uid,gaveUid:cand.uid}; } function handOkeyPenalty(){return okeyColorPenalty()} function tilePenaltyAmount(t){if(!t)return 0;var v=CFG.COLOR_PENALTY_RULES[t.color];return typeof v==="number"?v:tv(t)*10}
