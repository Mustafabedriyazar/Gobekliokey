...
var WORKABLE_DISCARD_PENALTY=250;var OKEY_DISCARD_PENALTY=250;var WORKABLE_SIDE_TAKE_PENALTY=250;
...
function badProcessPenalty(p,uid,meldId,reason){
  /* v192 KANON — hatali islek denemesi CEZA DEGILDIR: tas istakaya doner, state degismez, ledger yazilmaz. */
  return{ok:false,err:"hatalı işlek: taş ıstakaya döner, ceza yok",amount:0,penalty:null,uid:uid,meldId:meldId,reason:reason||""};
}
...
function check(){
  ...
  var specialErr=[];
  if(st.finishSpecial){
    var fs=st.finishSpecial,bc=(fs.pairFinish?1:0)+(fs.okeyFinish?1:0),wantMul=(fs.pairFinish?4:1)*(fs.okeyFinish?8:1);
  ...
  if(st.gameFinished){
    var mf=st.matchFinal;if(!mf||!mf.rows||mf.rows.length!==4)matchErr.push("missing_match_final");
      ...
      if(mf.handsCounted<CFG.END_AFTER_HANDS)matchErr.push("match_hands_counted:"+mf.handsCounted);
    }else{
...
function a_take(p){
  ...
  var _wk=workableDiscardTargets(cd.tile).length>0;
  st.discardPile.pop();st.currentDiscard=st.discardPile.length?st.discardPile[st.discardPile.length-1]:null;
  st.pending={tile:cd.tile,by:cd.by,workable:_wk,byPairOpener:st.players[cd.by].opened&&st.players[cd.by].openingType==="PAIR"};
  st.players[p].hasDrawn=true;st.turnState="ACTION";
  ev("TAKE_DISCARD",p,{uid:cd.tile.uid,from:cd.by});
  return{ok:true,tile:cd.tile};
}
function a_okeyTake(p,meldId,candUid,jokUid){ var g=guard(p,["ACTION"]);if(g)return{ok:false,err:g}; var P=st.players[p];if(!P||!P.hasDrawn)return{ok:false,err:"once tas cekmelisin"}; var m=null;for(var i=0;i<st.melds.length;i++)if(st.melds[i].id===meldId){m=st.melds[i];break} if(!m)return{ok:false,err:"per bulunamadi"}; var ci=-1;for(i=0;i<P.rack.length;i++)if(String(P.rack[i].uid)===String(candUid)){ci=i;break} if(ci<0)return{ok:false,err:"tas elinde degil"}; var cand=P.rack[ci]; if(isJok(cand,st))return{ok:false,err:"okey ile okey alinmaz"}; var ji=-1;for(i=0;i<m.tiles.length;i++)if(isJok(m.tiles[i],st)){ji=i;break} if(ji<0)return{ok:false,err:"perde okey yok"}; if(jokUid!=null){var jf=-1;for(i=0;i<m.tiles.length;i++)if(String(m.tiles[i].uid)===String(jokUid)&&isJok(m.tiles[i],st)){jf=i;break} if(jf<0)return{ok:false,err:"hedef okey bulunamadi"}; ji=jf;} var jok=m.tiles[ji]; var _rep=jok.rep||null; if(!_rep){var _dv=(m.kind==="pair")?grpValid(m.tiles):tableMeldValid(m);var _dr=jokerReps(m.tiles,_dv);_rep=(_dr&&_dr[jok.uid])||null;if(_rep)jok.rep={color:_rep.color,num:_rep.num}} if(_rep){var _cf=meldFace(cand,st);if(!_cf||_cf.color!==_rep.color||_cf.num!==_rep.num)return{ok:false,err:"yalniz okeyin temsil ettigi dogal tas ile degistirilebilir"}} var trial=m.tiles.slice();trial[ji]=cand; var ok2=false; if(m.kind==="series"){var tv2=tableMeldValid({id:m.id,kind:"series",tiles:trial,openLen:m.openLen});ok2=!!(tv2&&tv2.kind==="series");if(!ok2){var gv2=grpValid(trial);ok2=!!(gv2&&gv2.kind!=="pair"&&(gv2.form==="male"||gv2.form==="female"||gv2.kind==="series"))}} else if(m.kind==="pair"){var gv=grpValid(trial);ok2=!!(gv&&gv.kind==="pair")} if(!ok2)return{ok:false,err:"aday tas okeyin yerini legal dolduramiyor"}; m.tiles[ji]=cand;P.rack.splice(ci,1);P.rack.push(jok);P.okeyLockUid=jok.uid;P.okeyLockKey=st.handIndex+":"+st.turnCount; ev("OKEY_TAKE",p,{meld:m.id,slot:ji,inUid:cand.uid,outUid:jok.uid}); return{ok:true,meld:m.id,slot:ji,tookUid:jok.uid,gaveUid:cand.uid}; } function handOkeyPenalty(){return okeyColorPenalty()} function tilePenaltyAmount(t){if(!t)return 0;var v=CFG.COLOR_PENALTY_RULES[t.color];return typeof v==="number"?v:tv(t)*10}
...
function a_process(p,meldId,uids){
  ...
  if(Array.isArray(uids)&&uids.length===1){var _mm=null,_qq;for(_qq=0;_qq<st.melds.length;_qq++)if(st.melds[_qq].id===meldId){_mm=st.melds[_qq];break} if(_mm){var _ck=findT(p,uids[0]); if(_ck>=0){var _cc=st.players[p].rack[_ck]; if(!isJok(_cc,st)){var _ff=meldFace(_cc,st); if(_ff){for(_qq=0;_qq<_mm.tiles.length;_qq++){var _jj=_mm.tiles[_qq]; if(!isJok(_jj,st))continue; var _rr=_jj.rep||null; if(!_rr){var _vv=(_mm.kind==="pair")?grpValid(_mm.tiles):tableMeldValid(_mm);var _gg=jokerReps(_mm.tiles,_vv);_rr=(_gg&&_gg[_jj.uid])||null;if(_rr)_jj.rep={color:_rr.color,num:_rr.num}} if(_rr&&_rr.color===_ff.color&&_rr.num===_ff.num){var _oo=a_okeyTake(p,meldId,uids[0],_jj.uid); if(_oo&&_oo.ok){_oo.replaced=true;return _oo} break}}}}}}}
  ...
  if(m.kind==="pair"){
    ...
    var takePenaltyPair=null;if(usedPendPair&&st.pending){var ppd=st.pending;st.pending=null;takePenaltyPair=pen(p,p,[ppd.tile],ppd.workable?"WORKABLE_SIDE_TAKE":"DISCARD_TAKEN_USED",ppd.workable?WORKABLE_SIDE_TAKE_PENALTY:handOkeyPenalty());takePenaltyPair.from=ppd.by;takePenaltyPair.label="ISLEK CEZASI"}
  ...
  var takePenalty=null;if(usedPend&&st.pending){var pd=st.pending;st.pending=null;takePenalty=pen(p,p,[pd.tile],pd.workable?"WORKABLE_SIDE_TAKE":"DISCARD_TAKEN_USED",pd.workable?WORKABLE_SIDE_TAKE_PENALTY:handOkeyPenalty());takePenalty.from=pd.by;takePenalty.label="ISLEK CEZASI"}
...
function a_discard(p,uid){
  ...
  if(st.pending){if(String(uid)===String(st.pending.tile.uid))return a_takeCancel(p);var _pd=st.pending;st.pending=null;P.rack.push(_pd.tile);P.hasDrawn=true;keptPending=true;takePenalty=pen(p,p,[_pd.tile],"SIDE_COMMIT",handOkeyPenalty());takePenalty.from=_pd.by;takePenalty.label="İŞLEK CEZASI";ev("SIDE_COMMIT",p,{uid:_pd.tile.uid,amount:takePenalty.amount});}if(!P.hasDrawn)return{ok:false,err:"önce taş çekmelisin"};
...
function finishSpecialMeta(winner,finishTile){
  ...
  meta.multiplier=(meta.pairFinish?4:1)*(meta.okeyFinish?8:1);
  return meta;
}
...
function matchTieKey(r){return [r.totalPenalty,-r.handWins,r.majorCount].join("|")}
...
function buildMatchFinal(){
  ...
  if(isTeamMode()){
    ...
    teams.sort(function(a,b){return (a.totalPenalty-b.totalPenalty)||(b.handWins-a.handWins)||(a.majorCount-b.majorCount)||(a.team-b.team)});
    ...
    var outT={teamMode:true,rows:rows,teamRows:teams,championTeams:championTeams,championTeam:championTeams.length===1?championTeams[0]:null,tie:championTeams.length>1,handsPlayed:st.handIndex+1,handsCounted:st.handCount,rule:"LOWEST_TEAM_TOTAL",tieRule:"TEAM_TOTAL > TEAM_WINS > FEWER_TEAM_MAJOR; EXACT=TIE"};
    st.matchFinal=outT;return outT;
  }
  rows.sort(function(a,b){return (a.totalPenalty-b.totalPenalty)||(b.handWins-a.handWins)||(a.majorCount-b.majorCount)||(a.seat-b.seat)});
