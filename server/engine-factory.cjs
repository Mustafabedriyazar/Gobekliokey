...
function a_discard(p,uid){
  ...
  if(st.pending){if(String(uid)===String(st.pending.tile.uid))return a_takeCancel(p);var _pd=st.pending;st.pending=null;P.rack.push(_pd.tile);P.hasDrawn=true;keptPending=true;takePenalty={type:"SIDE_COMMIT",amount:handOkeyPenalty(),tiles:[_pd.tile.uid],label:"İŞLEK CEZASI"};P.handPenalty=(P.handPenalty||0)+takePenalty.amount;ev("SIDE_COMMIT",p,{uid:_pd.tile.uid,amount:takePenalty.amount});}if(!P.hasDrawn)return{ok:false,err:"önce taş çekmelisin"};
