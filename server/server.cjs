...
function readyReport(){const reasons=[...CFG.errors];if(!runtimeReady)reasons.push(runtimeError?'STORE_INIT_FAILED':'STORE_INITIALIZING');if(!CFG.authDisabled&&!identityReady)reasons.push(identityError?'IDENTITY_INIT_FAILED':'IDENTITY_INITIALIZING');if(!matchmakingReady)reasons.push(matchmakingError?'MATCHMAKING_INIT_FAILED':'MATCHMAKING_INITIALIZING');if(!casualMatchmakingReady)reasons.push(casualMatchmakingError?'CASUAL_MATCHMAKING_INIT_FAILED':'CASUAL_MATCHMAKING_INITIALIZING');if(draining)reasons.push('DRAINING');if(CFG.storeMode==='redis'&&!store.connected)reasons.push('REDIS_NOT_CONNECTED');if(!CFG.authDisabled&&CFG.storeMode==='redis'&&!identityStore.connected)reasons.push('IDENTITY_REDIS_NOT_CONNECTED');if(CFG.storeMode==='redis'&&!matchmakingStore.connected)reasons.push('MATCHMAKING_REDIS_NOT_CONNECTED');if(CFG.storeMode==='redis'&&!casualMatchmakingStore.connected)reasons.push('CASUAL_MATCHMAKING_REDIS_NOT_CONNECTED');return{ok:reasons.length===0,reasons,build:'gobek17-196-grid-dragdrop-kanon',instanceId:CFG.instanceId,store:CFG.storeMode,replicas:CFG.replicaCount,authMode:CFG.authMode,moderation:CFG.moderationEnabled,rankedK:RANKED_K}}
...
const server=http.createServer(async(req,res)=>{if(mediaRoute(req,res))return;if(diagRoute(req,res))return;
  ...
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/health/live')return json(req,res,200,{ok:true,service:'G17 authority',build:'gobek17-196-grid-dragdrop-kanon',instanceId:CFG.instanceId,uptimeMs:Date.now()-metrics.startedAt});
