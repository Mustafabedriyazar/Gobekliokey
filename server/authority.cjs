...
class AuthoritativeRoom{
  ...
  snapshotForSeat(viewSeat){
    ...
    const snap={protocol:'G17MP/1',roomId:this.id,rev:this.rev,eventSeq:this.eventSeq,mode:this.mode,context:this.context,started:this.started,serverTime:now(),audit:this._publicAudit(),lobby,rankedResult:this.rankedResult?clone(this.rankedResult):null,
      hand:{index:st.handIndex,type:st.handType,handCount:st.handCount,bigHandCount:st.handCount,dealer:st.dealer,starter:st.starter,turnIndex:st.turnIndex,turnCount:st.turnCount,turnState:st.turnState,firstRoundActive:st.firstRoundActive,handOver:st.handOver,winner:st.winner,gameFinished:st.gameFinished,deckCount:st.deck.length,discardPileCount:st.discardPile?st.discardPile.length:0,lastOpenTotal:st.lastOpenTotal,pairSeriesMin:st.pairSeriesMin||0,okeyMode:st.okeyMode,indicator:tile(st.indicator),okey:st.okey?{color:st.okey.color,num:st.okey.num}:null},
      ...
      melds:st.melds.map(m=>({id:m.id,owner:m.owner,kind:m.kind,form:m.form,color:m.color,openLen:m.openLen==null?m.tiles.length:m.openLen,processAdds:m.processAdds||0,gst:m.gst||null,tiles:m.tiles.map(tile)})),ledger,chat:{seq:this.chatSeq,messages:this.chat.slice(-30).map(clone)},
