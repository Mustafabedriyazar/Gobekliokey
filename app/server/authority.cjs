...
class AuthoritativeRoom{
  ...
  join(name,preferredSeat,suppliedToken,clientJoinId,identity){
    ...
    if(this.context==='RANKED'&&this.matchmakingId){
      if(!accountId)return{ok:false,err:'AUTH_REQUIRED'};
      if(this.matchmakingExpiresAt&&now()>this.matchmakingExpiresAt)return{ok:false,err:'RANKED_MATCH_EXPIRED'};
      const reserved=this.allowedAccounts.indexOf(accountId);
      if(reserved<0)return{ok:false,err:'RANKED_ACCOUNT_NOT_MATCHED'};
      if(this.seats[reserved]&&this.seats[reserved].token&&this.seats[reserved].accountId!==accountId)return{ok:false,err:'RANKED_SEAT_TAKEN'};
  ...
  _dispatchEngine(seat,a){
    ...
    switch(a.type){
      ...
      case 'PROCESS': return E.process(seat,String(a.meldId||''),Array.isArray(a.uids)?a.uids:[]);
