...
let draining=false,runtimeReady=false,runtimeError=null,identityReady=false,identityError=null,matchmakingReady=false,matchmakingError=null;
...
function readyReport(){const reasons=[...CFG.errors];if(!runtimeReady)reasons.push(runtimeError?'STORE_INIT_FAILED':'STORE_INITIALIZING');if(!CFG.authDisabled&&!identityReady)reasons.push(identityError?'IDENTITY_INIT_FAILED':'IDENTITY_INITIALIZING');if(!matchmakingReady)reasons.push(matchmakingError?'MATCHMAKING_INIT_FAILED':'MATCHMAKING_INITIALIZING');if(draining)reasons.push('DRAINING');if(CFG.storeMode==='redis'&&!store.connected)reasons.push('REDIS_NOT_CONNECTED');if(!CFG.authDisabled&&CFG.storeMode==='redis'&&!identityStore.connected)reasons.push('IDENTITY_REDIS_NOT_CONNECTED');if(CFG.storeMode==='redis'&&!matchmakingStore.connected)reasons.push('MATCHMAKING_REDIS_NOT_CONNECTED');return{ok:reasons.length===0,reasons,build:'gobek17-177-v177-inspect-deste-bitti-el-sonu-akisi-mşlek-taş-kuralı-workable-discard-ırakmada-insert-shift-yerleşiırakmada-insert-shift-yerleşi',instanceId:CFG.instanceId,store:CFG.storeMode,replicas:CFG.replicaCount,authMode:CFG.authMode,moderation:CFG.moderationEnabled,rankedK:RANKED_K}}
...
async function settleMatchProfile(room){
  ...
  try{
    ...
    if(ranked&&matchmaking)await matchmaking.completeMatch(room.matchmakingId);
    return fresh;
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/health/live')return json(req,res,200,{ok:true,service:'G17 authority',build:'gobek17-177-v177-inspect-deste-bitti-el-sonu-akisi-mşlek-taş-kuralı-workable-discard-ırakmada-insert-shift-yerleşiırakmada-insert-shift-yerleşi',instanceId:CFG.instanceId,uptimeMs:Date.now()-metrics.startedAt});
    ...
    if(parts[0]==='v1'&&parts[1]==='auth'){
      ...
      if(req.method==='POST'&&op==='login'){
        const b=await readJson(req),lk=ip+'|'+identitySha(String(b.username||'').toLowerCase()).slice(0,16),lr=loginLimiter.take(lk);if(!lr.ok){metrics.rateLimited++;return json(req,res,429,{ok:false,err:'RATE_LIMITED'})}
      ...
      if(req.method==='POST'&&op==='recover'){const b=await readJson(req),rk=ip+'|'+identitySha(String(b.username||'').toLowerCase()).slice(0,16),lim=recoveryLimiter.take(rk);if(!lim.ok){metrics.rateLimited++;return json(req,res,429,{ok:false,err:'RATE_LIMITED'})}const r=await identity.recover(b);if(r.ok){metrics.authRecovery++;audit.log('AUTH_RECOVER',{accountId:r.user&&r.user.id,ip})}else{metrics.authFail++;audit.log('AUTH_RECOVER_FAIL',{ip})}return json(req,res,r.ok?200:401,r)}
