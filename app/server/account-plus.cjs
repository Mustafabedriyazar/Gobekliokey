...
function makeAccountPlus(opts){
  ...
  async function summaryFor(accountId){
    const out = { build:'v183', account:null, wallet:null, league:null, ranked:null, stats:null, history:null, missing:[] };
  ...
  async function handle(req, res){
    ...
    try {
      if (p === '/v1/auth/config' && req.method === 'GET'){
        return send(res, 200, { build:'v183', googleClientId: env.GOOGLE_CLIENT_ID || null, googleReady: !!env.GOOGLE_CLIENT_ID, mailerReady: mailerConfigured(), passwordMin: parseInt(env.G17_PASSWORD_MIN||'10',10)||10 });
      }
