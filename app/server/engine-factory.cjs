...
function meldProcessAdds(m){var n=Number(m&&m.processAdds);return isFinite(n)&&n>=0?Math.floor(n):0}
...
function seriesEndpointPlan(m,tiles){
  ...
  var used=meldProcessAdds(m);if(used+tiles.length>2)return null;
...
function a_process(p,meldId,uids){
  ...
  var plan=seriesEndpointPlan(m,tiles);if(!plan){if(meldProcessAdds(m)+tiles.length>2)return{ok:false,err:"bu pere toplam en fazla 2 taş işlenebilir"};return{ok:false,err:"taş yalnız perin en soluna veya en sağına işlenebilir; açık per bölünmez/değişmez"}}
  ...
  var oldUids=m.tiles.map(function(t){return t.uid}),left=plan.left||[],right=plan.right||[];m.tiles=left.concat(m.tiles,right);m.processAdds=meldProcessAdds(m)+tiles.length;if(m.openLen==null)m.openLen=oldUids.length;
...
function a_discard(p,uid){
  ...
  var willFinish=P.rack.length===1,t=P.rack[ix];
...
function matchSeatStats(seat){
  ...
  var majorTypes={WORKABLE_DISCARD:1,OKEY_DISCARD:1,OKEY_HELD_END:1,BAD_OPEN_ATTEMPT:1};
